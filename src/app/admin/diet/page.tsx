...
export default function AdminDietPage() {
  ...
  return (
    <section aria-label="Admin diet plan management" className="space-y-5 lg:space-y-8">
      {/* Header */}
      <header className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h2 className="text-2xl lg:text-4xl font-bold">Diet Plans</h2>
          <p className="text-sm lg:text-base text-slate-500 mt-1">Manage weekly meal schedules</p>
        </div>
      ...
      <div className="lg:hidden space-y-3">
        {weeklyPlan[selectedDay]?.map((meal) => (
          <div key={meal.id} className="glass-card-strong p-4 shadow-depth">
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              <div>
                <div className="font-medium text-slate-700">{meal.protein}g</div>
                <div className="text-slate-500">Protein</div>
              </div>
              <div>
                <div className="font-medium text-slate-700">{meal.carbs}g</div>
                <div className="text-slate-500">Carbs</div>
              </div>
              <div>
                <div className="font-medium text-slate-700">{meal.fats}g</div>
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button 
                ...
                className="flex-1 px-3 py-2 rounded-xl bg-slate-100 text-slate-600 text-xs font-medium cursor-pointer"
              >
                Remove
              </button>
            </div>
        ...
        <button 
          ...
          className="w-full py-3 rounded-2xl border-2 border-dashed border-slate-300 text-slate-600 text-sm font-medium hover:border-iosBlue hover:text-iosBlue transition-all cursor-pointer"
        >
          + Add Meal
        </button>
      </div>
      ...
      <div className="hidden lg:block ios-glass rounded-3xl overflow-hidden">
          ...
          <table className="w-full min-w-[1200px]">
            <thead>
              <tr className="bg-white/40 border-b border-slate-200/50">
                <th className="px-4 py-4 text-left text-sm font-semibold text-slate-700 w-32">Meal Type</th>
                {daysOfWeek.map((day) => (
                  <th key={day} className="px-4 py-4 text-center text-sm font-semibold text-slate-700">
                    {day}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {['Breakfast', 'Lunch', 'Dinner', 'Snack'].map((mealType) => (
                <tr key={mealType} className="border-b border-slate-200/30">
                  <td className="px-4 py-3 font-medium text-slate-700 text-sm">{mealType}</td>
                  {daysOfWeek.map((day) => {
                    const dayMeals = weeklyPlan[day] || [];
                      ...
                      <td key={day} className="px-2 py-3">
                        {meal ? (
                          <div className="glass-chip p-3 hover:shadow-depth transition-all cursor-pointer">
                            <div className="font-medium text-slate-800 text-sm mb-1">{meal.name}</div>
                            <div className="text-xs text-slate-600 mb-2">{meal.calories} kcal</div>
        ...
        <div className="bg-white/20 border-t border-slate-200/50 p-4">
          <div className="grid grid-cols-8 gap-4">
            <div className="font-semibold text-slate-700 text-sm">Daily Totals</div>
            {daysOfWeek.map((day) => {
      ...
      <div className="glass-card-strong p-5 lg:p-8">
        ...
        {(['Breakfast', 'Lunch', 'Dinner', 'Snack'] as const).map(mealType => (
          <div key={mealType} className="mb-6 last:mb-0">
            <h4 className="text-sm font-medium text-slate-600 mb-3">{mealType} Options</h4>
              ...
              {mealLibrary.filter(m => m.type === mealType).map((meal) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h4 className="font-semibold text-slate-800 text-sm">{meal.name}</h4>
