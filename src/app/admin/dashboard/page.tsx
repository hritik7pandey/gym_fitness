...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #B9D7FF 0%, #D8C7FF 35%, #A6F1FF 70%, #E5E1FF 100%)' }}>
      {/* iOS 26 Ambient Glow Blobs */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-[-8%] right-[-4%] w-[500px] h-[500px] rounded-full opacity-30 animate-float" style={{ background: 'radial-gradient(circle, rgba(185, 215, 255, 0.6) 0%, rgba(216, 199, 255, 0.3) 60%, transparent 100%)', filter: 'blur(48px)' }}></div>
        <div className="absolute bottom-[-10%] left-[-4%] w-[600px] h-[600px] rounded-full opacity-25 animate-float" style={{ background: 'radial-gradient(circle, rgba(166, 241, 255, 0.5) 0%, rgba(229, 225, 255, 0.25) 60%, transparent 100%)', filter: 'blur(48px)', animationDelay: '2s' }}></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8 relative z-10">
        ...
        <header className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-[32px] lg:text-[40px] font-[800] text-[#1C1C1E]" style={{ fontFamily: 'SF Pro Display, -apple-system, sans-serif' }}>
                Admin Dashboard
              </h1>
              <p className="text-[15px] lg:text-[17px] font-[600] text-[#1C1C1E]/70 mt-1" style={{ fontFamily: 'SF Pro, -apple-system, sans-serif' }}>Manage your fitness hub</p>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {systemStats.map((stat, index) => (
            <div key={index} className="glass-card p-6 hover:shadow-depth transition-all">
              ...
              <div className="text-sm text-[#1C1C1E]/70 font-medium">{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="glass-card-strong p-6 mb-8">
          <h3 className="gradient-title text-xl mb-4">Quick Actions</h3>
              ...
              {recentUsers.map((user, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-[#1C1C1E]">{user.name}</h4>
                      <p className="text-sm text-[#1C1C1E]/60">{user.email}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-xs text-[#1C1C1E]/50 mt-1">{user.joined}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              {[
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-[#1C1C1E]">{activity.action}</p>
                    <p className="text-xs text-[#1C1C1E]/50">{activity.time}</p>
                  </div>
                </div>
              ))}
            </div>
                ...
                return (
                  <div key={month}>
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-medium text-[#1C1C1E]/80">{month}</span>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={index} className="p-4 glass-chip">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <h4 className="font-semibold text-[#1C1C1E]">{item.plan}</h4>
                      <p className="text-sm text-[#1C1C1E]/60">{item.users} active subscriptions</p>
                    </div>
