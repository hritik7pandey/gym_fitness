  ...
  return (
    <GlassCard title="Diet Plan" subtitle={planSummary}>
      <div className="space-y-4 lg:space-y-5">
        <div className="text-sm lg:text-base text-gray-600 font-medium">Tap to view meals and swap items.</div>
        <div className="grid grid-cols-3 gap-3 lg:gap-6">
          {macros.map((macro, i) => (
            <div key={macro.name} className="text-center group">
              ...
              <div className="text-sm lg:text-base font-bold text-gray-900">{macro.value}</div>
              <div className="text-xs lg:text-sm text-gray-600 font-semibold mt-0.5">{macro.name}</div>
            </div>
          ))}
        </div>
      </div>
